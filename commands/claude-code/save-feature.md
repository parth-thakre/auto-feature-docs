...
**Quick flow:**
1. Get name from $1 or git branch (bash: `git branch --show-current 2>/dev/null`)
2. Clean branch name: remove feat/, hotfix/ prefixes, convert to kebab-case
3. Create `docs/features/{kebab-name}/` if needed (bash: `mkdir -p`)
4. Pick filename: `YYYY-MM-DD.md` (e.g., 2026-02-02.md) or `YYYY-MM-DD-v2.md` if exists
5. Analyze recent changes (bash: `git diff --name-only HEAD~5 2>/dev/null`)
